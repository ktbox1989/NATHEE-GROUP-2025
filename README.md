...
- Public Website source now has 11 clean, SEO-specific routes. Its Gallery consumes a versioned, privacy-checked release manifest with the first two Owner-supplied company-work photographs and never substitutes stock imagery for real work.
...
- A structured Site Content CMS is implemented for Home, Services, About and Contact with immutable revisions, audited publication/rollback, safe section validation and real Gallery media selection. The Gallery admin also supports bounded 20-image sequential batches. These features are source/build complete but not Production-live until Auth, D1 migration `0012`, R2 and application routing pass their gates.
