...
export type PublicGalleryItem = { id: string; title: string; caption: string | null; altText: string; categoryName: string; takenAt: string | null; location: string | null; thumbnailSrc?: string; displaySrc?: string; width?: number; height?: number };
...
export function GalleryLightbox({ items }: { items: PublicGalleryItem[] }) {
  ...
  return <>
    <div className="public-gallery-grid">{items.map((item, index) => <figure key={item.id}>
      <button type="button" onClick={(event) => { setOpener(event.currentTarget); setSelected(index); }} aria-label={`เปิดภาพขนาดใหญ่: ${item.altText}`}><img src={item.thumbnailSrc ?? `/api/gallery/images/${item.id}?role=thumbnail`} alt={item.altText} width={item.width ?? 640} height={item.height ?? 480} loading="lazy" decoding="async" sizes="(max-width: 600px) calc(100vw - 28px), (max-width: 940px) 48vw, 32vw" /></button>
