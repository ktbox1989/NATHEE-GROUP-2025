...
import { isSitePageSlug } from "@/lib/site-cms";
...
export async function POST(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  ...
  if (action === "PUBLISH") {
    const revision = await db.select({ id: sitePageRevisions.id }).from(sitePageRevisions).where(and(eq(sitePageRevisions.id, revisionId!), eq(sitePageRevisions.pageId, page.id))).get();
  ...
  try {
    await db.batch([
      ...
      db.insert(auditLogs).values(makeAuditRecord({ actor, action, entityType: "site_page_publication", entityId: eventId, after: { slug, revisionId: action === "PUBLISH" ? revisionId : null, note } })),
    ]);
  } catch {
