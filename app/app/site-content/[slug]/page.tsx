...
type Props = { params: Promise<{ slug: string }>; searchParams: Promise<{ status?: string; error?: string; revision?: string }> };
...
export default async function SitePageEdit({ params, searchParams }: Props) {
  ...
  return <><div className="app-page-head"><div><p>WEBSITE CMS</p><h1>{SITE_PAGE_DEFINITIONS[slug].label}</h1><span>{SITE_PAGE_DEFINITIONS[slug].path} · บันทึก Revision ก่อน แล้วจึงเลือกเผยแพร่</span></div><Link className="button button-glass" href="/app/site-content">← ทุกหน้า</Link></div>{query.status && <div className="form-message success page-message">ดำเนินการสำเร็จ: {query.status}</div>}{query.error && <div className="form-message error page-message">ไม่สำเร็จ: {query.error}</div>}
    <section className="app-panel cms-publication-panel"><div><span className={`status-pill ${publication?.action ?? "DRAFT"}`}>{publication?.action ?? "DRAFT"}</span><h2>สถานะเผยแพร่</h2><p>{publication?.action === "PUBLISH" ? `เผยแพร่ Revision ${publication.revisionId?.slice(0, 8)}…` : publication?.action === "HIDE" ? "ซ่อนหน้าเว็บแล้ว" : "ยังใช้หน้า Default ที่มากับระบบ"}</p></div><div>{selected && <Link className="button button-glass" href={`/app/site-content/${slug}/preview?revision=${encodeURIComponent(selected.id)}`}>ดูตัวอย่าง Revision</Link>}{canPublish && selected && publication?.revisionId !== selected.id && <PublishForm slug={slug} action="PUBLISH" revisionId={selected.id} label="เผยแพร่ Revision นี้" />}{canPublish && slug !== "home" && publication?.action === "PUBLISH" && <PublishForm slug={slug} action="HIDE" revisionId="" label="ซ่อนหน้านี้" />}</div></section>
    ...
    <section className="detail-section"><div className="detail-section-head"><div><p>REVISION HISTORY</p><h2>ประวัติย้อนหลัง</h2></div><span>แสดงล่าสุด {revisions.length} รายการ</span></div><div className="cms-revision-list">{revisions.map((revision) => <article className="app-panel" key={revision.id}><div><b>{revision.id.slice(0, 8)}…</b>{publication?.revisionId === revision.id && publication.action === "PUBLISH" && <span className="status-pill PUBLISH">LIVE</span>}</div><p>{revision.changeNote || "ไม่มีหมายเหตุ"}</p><small>{revision.author} · {new Date(revision.createdAt).toLocaleString("th-TH")} · SHA {revision.contentHash.slice(0, 12)}…</small><div><Link href={`/app/site-content/${slug}?revision=${encodeURIComponent(revision.id)}`}>เปิดแก้จาก Revision นี้</Link><Link href={`/app/site-content/${slug}/preview?revision=${encodeURIComponent(revision.id)}`}>ดูตัวอย่าง</Link>{canPublish && publication?.revisionId !== revision.id && <PublishForm slug={slug} action="PUBLISH" revisionId={revision.id} label="ย้อนกลับมาเผยแพร่" />}</div></article>)}</div>{!revisions.length && <div className="app-panel app-empty"><h2>ยังไม่มี Revision</h2><p>หน้าเว็บยังใช้ค่า Default ที่ตรวจผ่านใน Source</p></div>}</section>
  </>;
}

function PublishForm({ slug, action, revisionId, label }: { slug: string; action: "PUBLISH" | "HIDE"; revisionId: string; label: string }) {
  return <form className="cms-inline-form" action={`/api/site-content/${slug}/publish`} method="post"><input type="hidden" name="action" value={action} /><input type="hidden" name="revisionId" value={revisionId} /><input type="hidden" name="requestKey" value={`cms-publish-${crypto.randomUUID()}`} /><button type="submit">{label}</button></form>;
}
