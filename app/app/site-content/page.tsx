...
export default async function SiteContentPage() {
  ...
  return <><div className="app-page-head"><div><p>WEBSITE CMS</p><h1>จัดการหน้าเว็บไซต์</h1><span>แก้ไขแบบ Revision ก่อนเผยแพร่ ไม่มีการเขียน HTML หรือสคริปต์ลงหน้าเว็บโดยตรง</span></div><div className="app-page-actions"><Link className="button button-glass" href="/app/website">ภาพรวมเว็บไซต์</Link><Link className="button button-glass" href="/app/site-settings">ตั้งค่าโลโก้ เมนู และ Footer</Link></div></div><section className="site-page-grid">{rows.map((row) => <article className="app-panel" key={row.slug}><span className={`status-pill ${row.state}`}>{row.state}</span><h2>{row.definition.label}</h2><p>{row.definition.path}</p><small>{row.revisionCount ? `${row.revisionCount}${row.revisionCount >= 100 ? "+" : ""} revisions` : row.state === "UNMANAGED" ? "ยังไม่มี record ใน D1 · การบันทึกครั้งแรกจะสร้างเป็น Draft" : "ยังไม่มี Revision"}{row.updatedAt ? ` · อัปเดต ${new Date(row.updatedAt).toLocaleString("th-TH")}` : ""}</small><div><Link className="button button-gradient" href={`/app/site-content/${row.slug}`}>{row.state === "UNMANAGED" ? "เพิ่มหน้านี้" : "แก้ไขหน้า"}</Link><Link className="button button-glass" href={row.definition.path}>เปิดหน้าเว็บ</Link></div></article>)}</section><section className="app-panel cms-safety-note"><h2>รูปและผลงาน</h2><p>รูปทั้งหมดจัดการผ่าน Media Library เพื่อคงหมวดหมู่ Alt text ลำดับ ภาพเด่น และสถานะเผยแพร่</p><Link className="button button-glass" href="/app/gallery">เปิด Media Library</Link></section><section className="app-panel cms-safety-note"><h2>ขอบเขตความปลอดภัย</h2><p>ระบบเพิ่มได้เฉพาะหน้าที่อยู่ในรายการที่อนุญาตข้างต้น Slug จึงผ่านการตรวจตั้งแต่ก่อนเปิด Editor และ D1 ป้องกัน record ซ้ำ เนื้อหาถูก escape อัตโนมัติ ภาพต้องมาจาก Gallery ที่เผยแพร่แล้ว และทุกการบันทึก/เผยแพร่มี Audit กับประวัติย้อนกลับ</p></section></>;
}
