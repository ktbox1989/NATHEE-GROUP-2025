...
export default async function SiteSettingsPage({ searchParams }: Props) {
  ...
  return <><div className="app-page-head"><div><p>WEBSITE SETTINGS</p><h1>ตั้งค่าเว็บไซต์ส่วนกลาง</h1><span>ชื่อแบรนด์ โลโก้ เบอร์โทร เมนู และ Footer ใช้ร่วมกันทุกหน้าสาธารณะ</span></div><div className="app-page-actions">{canWrite && <Link className="button button-gradient" href="#site-settings-editor">แก้ไขการตั้งค่า</Link>}<Link className="button button-glass" href="/app/site-content">แก้ไข SEO รายหน้า</Link></div></div>{query.status && <div className="form-message success page-message">ดำเนินการสำเร็จ: {query.status}</div>}{query.error && <div className="form-message error page-message">{cmsErrorMessage(query.error, query.missing)}</div>}
