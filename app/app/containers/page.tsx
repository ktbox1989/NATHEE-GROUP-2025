...
export default async function ContainersPage({ searchParams }: Props) {
  ...
  return <>
    ...
    <section className="detail-section"><div className="detail-section-head"><div><p>CONTAINER REGISTRY</p><h2>ตู้ล่าสุด</h2></div><span>{rows.length} รายการในหน้านี้</span></div>
      {rows.length ? <div className="container-grid">{rows.map((container) => <article className="app-panel container-card" key={container.id}><div><b>{container.containerNumber}</b><span className="status-pill">{container.status}</span></div><h2>{container.type} · {container.port}</h2><p>ปลายทาง {container.country}</p><dl><div><dt>Seal</dt><dd>{container.sealNumber || "ยังไม่ระบุ"}</dd></div><div><dt>ความจุ</dt><dd>{container.capacityMotorcycles ?? "ยังไม่ยืนยัน"} คัน</dd></div></dl><small>สร้าง {formatThaiDateTime(container.createdAt)}</small></article>)}</div> : <div className="app-panel app-empty"><div>🚢</div><h2>ยังไม่มีทะเบียนตู้</h2><p>เพิ่มเฉพาะเลขตู้จริงที่ผ่าน ISO 6346 check digit</p></div>}
    ...
    <div className="login-notice page-message">ขณะนี้เปิดเฉพาะทะเบียนตู้สถานะร่าง การผูกรถ, รูป, เอกสาร และ lifecycle จะเปิดเมื่อ Migration ถัดไปมี invariant และ Audit ครบ ห้ามใช้หน้านี้แทน Container Load Manifest</div>
  </>;
}
