...
export default async function GalleryAdminPage({ searchParams }: Props) {
  ...
  return <>
    <div className="app-page-head"><div><p>MEDIA LIBRARY</p><h1>Gallery / Portfolio</h1><span>จัดการภาพจริง คำบรรยาย สิทธิ์การมองเห็น และสถานะเผยแพร่ โดยไม่ปะปนกับหลักฐานงานลูกค้า</span></div><div className="app-page-actions">{canWrite && <Link className="button button-gradient" href="#gallery-upload">+ เพิ่มรายการ / อัปโหลดสื่อ</Link>}<Link className="button button-glass" href="/app/gallery/order">จัดลำดับ</Link></div></div>
    ...
    {canWrite && <section className="detail-section"><div className="detail-section-head"><div><p>CATEGORIES</p><h2>หมวดผลงาน</h2></div></div>
      <PendingForm className="record-form" action="/api/gallery/categories"><div className="field"><label htmlFor="category-name">ชื่อหมวด *</label><input id="category-name" name="name" maxLength={120} required /></div><div className="field"><label htmlFor="category-slug">Slug ภาษาอังกฤษ *</label><input id="category-slug" name="slug" pattern="[a-z0-9-]{2,80}" required placeholder="domestic-transport" /></div><div className="field"><label htmlFor="category-order">ลำดับ</label><input id="category-order" name="sortOrder" type="number" min={0} max={1000000} defaultValue={0} /></div><div className="field full"><label htmlFor="category-description">คำอธิบาย</label><input id="category-description" name="description" maxLength={500} /></div><div className="full"><PendingSubmitButton className="button button-gradient" busyLabel="กำลังเพิ่มหมวด…">เพิ่มหมวด</PendingSubmitButton></div></PendingForm>
      <div className="gallery-category-admin">{categories.map((category) => <PendingForm key={category.id} action={`/api/gallery/categories/${category.id}`} className="app-panel"><input name="name" defaultValue={category.name} maxLength={120} required aria-label="ชื่อหมวด" /><input name="slug" defaultValue={category.slug} pattern="[a-z0-9-]{2,80}" required aria-label="Slug" /><input name="description" defaultValue={category.description ?? ""} maxLength={500} aria-label="คำอธิบาย" /><input name="sortOrder" type="number" min={0} max={1000000} defaultValue={category.sortOrder} aria-label="ลำดับ" /><select name="status" defaultValue={category.status} aria-label="สถานะหมวด"><option value="ACTIVE">แสดง</option><option value="HIDDEN">ซ่อน</option></select><PendingSubmitButton busyLabel="กำลังบันทึกหมวด…">บันทึกหมวด</PendingSubmitButton></PendingForm>)}</div>
    </section>}

    {canWrite && <section className="detail-section" id="gallery-upload"><div className="detail-section-head"><div><p>BULK UPLOAD</p><h2>เพิ่มภาพจริงหลายภาพ</h2></div><span>ครั้งละไม่เกิน 20 ภาพ · อัปโหลดเป็น Draft เสมอ</span></div>{categories.some((category) => category.status === "ACTIVE") ? <GalleryBulkUploadForm categories={categories.filter((category) => category.status === "ACTIVE").map(({ id, name }) => ({ id, name }))} jobs={jobs.map((job) => ({ id: job.id, companyId: job.companyId, label: `${job.jobNumber} · ${job.companyName}` }))} /> : <div className="app-panel app-empty"><h2>สร้างหมวดก่อนอัปโหลด</h2><p>Gallery จะไม่รับภาพที่ไม่มีหมวด</p></div>}</section>}

    <section className="detail-section" id="gallery-library"><div className="detail-section-head"><div><p>LIBRARY</p><h2>รายการภาพ</h2></div><span>{items.length} รายการในหน้านี้</span></div>
...
function Action({ id, value, label }: { id: string; value: string; label: string }) { return <PendingForm action={`/api/gallery/${id}`} busyMessage={`กำลังดำเนินการ: ${label}`}><input type="hidden" name="action" value={value} /><PendingSubmitButton busyLabel="กำลังดำเนินการ…">{label}</PendingSubmitButton></PendingForm>; }
