...
export default async function PublicGalleryPage({ searchParams }: Props) {
  ...
  const db = getDb();
  const categories = await db.select({ id: galleryCategories.id, slug: galleryCategories.slug, name: galleryCategories.name }).from(galleryCategories).where(eq(galleryCategories.status, "ACTIVE")).orderBy(galleryCategories.sortOrder, galleryCategories.name).all();
  ...
  const hasMore = rows.length > GALLERY_PAGE_SIZE; const pageRows = rows.slice(0, GALLERY_PAGE_SIZE); const next = pageRows.at(-1);
  const items: PublicGalleryItem[] = pageRows.map(({ id, title, caption, altText, takenAt, location, categoryName }) => ({ id, title, caption, altText, takenAt, location, categoryName }));
