...
const required = [".htaccess", "404.html", "favicon.svg", "robots.txt", "sitemap.xml", "assets/site.css", "assets/site.js", "assets/gallery.json", "assets/brand/nathee-logo-display.jpg", "assets/brand/nathee-logo-display.webp", "assets/brand/nathee-logo-thumbnail.jpg", "assets/brand/nathee-logo-thumbnail.webp", "assets/contact/line-qr-owner-supplied.png", "site.webmanifest", "assets/brand/icon-192.png", "assets/brand/icon-512.png", "assets/brand/icon-maskable-512.png", "assets/brand/apple-touch-icon-180.png", "login/index.html", "login-status.html", "_nathee/news-gateway.php", "news/index.php", "assets/media/index.php", "sitemap.php", ...routes.map(routeFile)];
...
for (const file of files) if (["", ".html", ".css", ".js", ".json", ".php", ".webmanifest", ".txt", ".xml", ".svg"].includes(extname(file).toLowerCase())) text.set(relative(root, file).replaceAll("\\", "/"), await readFile(file, "utf8"));
...
for (const token of ["RewriteRule ^news/?$ news/index.php [L]", "RewriteRule ^news/[a-z0-9]+(?:-[a-z0-9]+)*/?$ news/index.php [L]", "RewriteRule ^sitemap\\.xml$ sitemap.php [L]", "RewriteRule ^assets/media/"]) if (!htaccess.includes(token)) throw new Error(`Local PHP News route missing: ${token}`);
if (/\[P(?:,|\])|\bProxyPass\b|SSLProxyEngine/i.test(htaccess)) throw new Error("Public release must not depend on Apache proxying.");
const phpGateway = text.get("_nathee/news-gateway.php");
for (const token of ["const NATHEE_NEWS_UPSTREAM_ORIGIN = 'https://app.natheegroup2025.com';", "CURLOPT_SSL_VERIFYPEER => true", "CURLOPT_SSL_VERIFYHOST => 2"]) if (!phpGateway.includes(token)) throw new Error(`PHP News gateway contract missing: ${token}`);
...
// The caching contract is what makes a publish visible at all.
//
// A page served with a long max-age means an editor publishes, the CMS reports
// success, and the live site keeps serving the previous revision with nothing
// anywhere reporting a failure. The sitemap and robots.txt are regenerated by
// the same publish, and with no Cache-Control a cache invents a freshness
// lifetime of its own — so a stale sitemap keeps advertising withdrawn URLs and
// hides the new ones. Assets may be cached because their bytes do not change
// without a deployment, but they still revalidate rather than being immutable.
const documentCache = htaccess.match(/<FilesMatch "[^"]*html[^"]*">\s*Header always set Cache-Control "([^"]+)"/);
if (!documentCache) throw new Error("No Cache-Control rule covers pages; a publish would not be visible.");
if (!documentCache[1].includes("no-cache") || !documentCache[1].includes("must-revalidate")) {
  throw new Error(`Pages must revalidate, got "${documentCache[1]}"`);
}
for (const extension of ["xml", "txt"]) {
  if (!documentCache[0].includes(extension)) {
    throw new Error(`Derived surfaces ending .${extension} are not covered by a revalidation rule.`);
  }
}
const assetCache = htaccess.match(/<FilesMatch "[^"]*css[^"]*">\s*Header always set Cache-Control "([^"]+)"/);
if (!assetCache || !assetCache[1].includes("must-revalidate")) {
  throw new Error("Assets must revalidate rather than being served immutably.");
}
