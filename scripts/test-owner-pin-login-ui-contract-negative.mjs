...
const CASES = [
  ...
  {
    name: "a refusal the PIN route actually sends loses its Thai message",
    apply: (d) => edit(d, LIB, (s) => s.replace("  invalid_pin:", "  invalidPin:")),
  },
  {
    name: "a PIN of the wrong shape is refused with no sentence to show for it",
    apply: (d) => edit(d, LIB, (s) => s.replace("  pin_format:", "  pinFormat:")),
  },
  {
    name: "the Owner-account conflict renders a blank card instead of saying the database must be fixed",
    apply: (d) => edit(d, LIB, (s) => s.replace("  owner_conflict:", "  ownerConflict:")),
  },
  {
    name: "the callback's origin refusal is left without copy",
    apply: (d) => edit(d, LIB, (s) => s.replace("  origin:", "  requestOrigin:")),
  },
  {
    name: "the PIN field is marked from the password door's codes, which never involved a PIN",
    apply: (d) => edit(d, PAGE, (s) => s.replace("isPinFieldRefusal(params.error)", 'params.error === "invalid_credentials"')),
  },
  {
    name: "a refusal that never compared a PIN starts marking the PIN field as invalid",
    apply: (d) => edit(d, LIB, (s) => s.replace('["pin_format", "invalid_pin"]', '["pin_format", "invalid_pin", "unavailable"]')),
  },
];
