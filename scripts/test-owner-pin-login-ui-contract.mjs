...
// Read off the routes that redirect to /login, not chosen by this screen.
// app/api/auth/owner-pin/login/route.ts answers config, pin_format,
// invalid_pin, too_many_attempts, unavailable and owner_conflict;
// lib/current-actor.ts answers config and not_authorized;
// app/api/auth/login/route.ts answers invalid_input and invalid_credentials;
// app/auth/callback/route.ts answers origin. A code missing from this list is a
// blank card after a refused login, which reads as "nothing happened".
const ERROR_CODES = [
  "config",
  "pin_format",
  "invalid_pin",
...
// The only two that are a statement about the value in the PIN field.
const PIN_FIELD_REFUSALS = ["pin_format", "invalid_pin"];
...
// aria-invalid is a claim about the value in the input. unavailable,
// owner_conflict and not_authorized never reached a comparison, and
// invalid_credentials belongs to the password door, so marking the field for
// any of them announces something untrue to a screen reader.
require(
  page.includes("isPinFieldRefusal(params.error)"),
  `${LOGIN_PAGE}: the PIN field must be marked from the route's own PIN refusals, not from a hand-written list of codes`,
);
require(
  lib.includes(`OWNER_PIN_FIELD_REFUSALS: readonly OwnerPinErrorCode[] = [${PIN_FIELD_REFUSALS.map((code) => `"${code}"`).join(", ")}];`),
  `${LOGIN_LIB}: only ${PIN_FIELD_REFUSALS.join(" and ")} may mark the PIN field; the rest never compared a PIN`,
);
