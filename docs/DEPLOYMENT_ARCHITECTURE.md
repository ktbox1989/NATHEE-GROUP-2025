...
| Database/migrations | `db/`, `drizzle/0000` through `0024` | A protected Sites D1 has the ten `0000` base tables, but no verified migration ledger and no `0001`–`0024` operational/authorization/CMS/settings/search-index/quotation-attachment/bulk-import/operational-QR/Audit-pagination/private-evidence/signed-POD/Auth-throttle/recovery-grant/Audit-immutability migrations | `DATABASE_NOT_PRODUCTION_VERIFIED` |
...
3. Keep the application private until `/api/health` returns HTTP 200 with
   ...
   `storage` all `true`.
4. Apply migrations `0000` through `0024` once, in order, with a migration
...
contain the required objects through migration `0024`, and R2 must answer a
...
- D1 has only the ten base tables; migration ledger, `0001`–`0024` Yard,
