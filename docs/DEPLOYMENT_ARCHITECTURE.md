...
| Database/migrations | `db/`, `drizzle/0000` through `0011` | A protected Sites D1 has the ten `0000` base tables, but no verified migration ledger and no `0001`–`0011` operational/authorization migrations | `DATABASE_NOT_PRODUCTION_VERIFIED` |
...
4. Apply migrations `0000` through `0011` once, in order, with a migration
...
- D1 has only the ten base tables; migration ledger, `0001`–`0011` Yard,
  Gallery, role, member, notification, trip, container, inspection/POD changes and R2 runtime readiness are
  not verified.
