...
The first notification slice is implemented locally. A real motorcycle status event creates recipient-scoped, idempotent in-app notifications in the same atomic mutation. Customer recipients are limited to their company; internal recipients require OWNER or explicit `status:read`; and the event actor is excluded. Inbox/read APIs are bounded and recipient-filtered. External LINE/email delivery, provider retries and consent policy remain pending, as do Trip/container, POD and bulk import.
