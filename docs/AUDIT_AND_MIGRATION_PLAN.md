...
The first notification slice is implemented locally. A real motorcycle status event creates recipient-scoped, idempotent in-app notifications in the same atomic mutation. Customer recipients are limited to their company; internal recipients require OWNER or explicit `status:read`; and the event actor is excluded. Inbox/read APIs are bounded and recipient-filtered. External LINE/email delivery, provider retries and consent policy remain pending, as do container, POD and bulk import.
...
The trip/motorcycle load slice is also implemented locally through additive migration `0008`. It records immutable assignment identity, prevents a motorcycle from being active on two trips, enforces company/capacity boundaries, and coordinates assignment state with both trip state and the existing audited motorcycle status. The trip flow never fabricates or silently advances motorcycle status. Cancellation and completion release the active assignment in the same D1 batch while retaining history. Containers, inspection/damage forms, POD, documents and bulk import remain pending.
