...
Inspection, damage, POD and per-motorcycle print/PDF records are implemented locally through migration `0011`. Database triggers require a passed receipt inspection before `INSPECTED` and an active same-motorcycle DELIVERY-evidence POD before `DELIVERED`. These migrations remain unapplied in Production; bulk import and external provider notifications remain pending.
