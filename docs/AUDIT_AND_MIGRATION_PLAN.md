...
Inspection, damage, POD and per-motorcycle print/PDF records are implemented locally through migration `0011`. Database triggers require a passed receipt inspection before `INSPECTED` and an active same-motorcycle DELIVERY-evidence POD before `DELIVERED`. Bounded CSV/XLSX motorcycle staging, zero-error reconciliation and atomic confirmation are implemented locally through migration `0017`; external provider notifications remain pending. These migrations remain unapplied in Production.
